...
def train_model(data, contamination):
...
def main(config_path):
    ...
    train_data, validation_data = train_test_split(data, test_size=config["model"]["test_size"], random_state=42)

    model = train_model(train_data, config["model"]["contamination"])
    ...
    with mlflow.start_run(
        experiment_id=mlflow.get_experiment_by_name(config["mlflow"]["experiment_name"]).experiment_id):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_path", default="config/training_config.yaml")
